...
_LEAD = [
    ...
    r"\bcontact\s*me\b", r"\breach\s*me\b", r"\bget\s*in\s*touch\b",
]
_BYE = [r"\b(bye|goodbye|see\s*ya|close\s*(the\s*)?session|end\s*(the\s*)?chat)\b"]

_EMAIL_RE = re.compile(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", re.IGNORECASE)
_PHONE_RE = re.compile(r"\b(?:\+?\d[\s\-()]*){7,}\d\b", re.IGNORECASE)

def _rx_any(pats): 
    rxs = [re.compile(p, re.IGNORECASE) for p in pats]
    return lambda q: any(r.search(q) for r in rxs)
...
def _contact_focus(q: str) -> Optional[str]:
    ...
    if re.search(r"\b(address|hq|location|office|where\s+(are\s+you\s+)?(based|located))\b", q, re.IGNORECASE): return "address"
...
def detect_intent(text: str) -> Tuple[str, Optional[str]]:
    ...
    if _is_bye(q): return "bye", None
    if _EMAIL_RE.search(q) or _PHONE_RE.search(q): return "lead", None
